...
import {
  ...
  akiraAbrirChat,
  akiraBorrarChat,
  // v1.10.0 — Deja constancia de la acción en el hilo. Sin esto la cita se
  // guarda pero la conversación sigue pidiendo la confirmación ya dada.
  akiraAnotarAccion
} from 'backend/akiraLogic.web';
// v1.9.0 — La ESCRITURA de las acciones de AKIRA pasa por aquí, no por el
// endpoint público. `ejecutarAccion` es Permissions.SiteMember: exige sesión
// de miembro del salón, que es justo lo que este page code sí tiene y
// /_functions/akiraAsk no.
import { ejecutarAccion } from 'backend/akiraEjecutorLogic.web';
...
// cuál se abre la pantalla. Valores válidos: 'asesor' | 'ayuda' | 'asistente'.
const MODO = 'asesor';
...
$w.onReady(async function () {
  ...
  // ── ACCIONES (plano ASISTENTE) ── v1.9.0
  //
  // AKIRA propone dentro del chat; la escritura ocurre AQUÍ, y solo después
  // de que una persona haya pulsado Confirmar en la tarjeta.
  //
  // POR QUÉ NO LO HACE EL BACKEND DE AKIRA: /_functions/akiraAsk es un
  // endpoint HTTP público y sin autenticar. Mientras AKIRA fue solo lectura
  // el riesgo era el coste; con escritura sería una puerta anónima a la
  // agenda de producción. `ejecutarAccion` es SiteMember y solo se alcanza
  // desde una página con sesión iniciada.
  //
  // `ejecutarAccion` NO se fía de lo que le llega: vuelve a preparar la
  // acción desde cero contra la agenda actual y solo escribe si sigue
  // saliendo limpia. Entre la propuesta y el clic pueden haber pasado
  // minutos, y en ese hueco cabe otra reserva.
  el.on('akira-ejecutar-accion', async (event) => {
    const { accion, params, sessionId, textoUsuario } = event.detail || {};
    if (!accion) {
      console.warn(`[${V}] akira-ejecutar-accion sin acción`);
      return;
    }
    console.log(`[${V}] ejecutando acción "${accion}"`);
    try {
      const result = await ejecutarAccion({ accion, params: params || {} });
      if (!result || !result.ok) {
        console.error(`[${V}] ejecutarAccion KO:`, JSON.stringify(result && (result.error || result.estado)));
      }
      trySetAttr('accionResultado', JSON.stringify(result || { ok: false, error: { message: 'Sin respuesta del servidor.' } }));

      // v1.10.0 — Anotar en la conversación lo que ha pasado DE VERDAD, para
      // que el siguiente turno no vuelva a pedir la confirmación. El texto lo
      // redacta el sistema con el resultado real, no el modelo.
      if (sessionId) {
        try {
          ...
          await akiraAnotarAccion({
            sessionId,
            textoUsuario: textoUsuario || 'Confirmar',
            textoResultado: texto
          });
        } catch (errAnota) {
          console.warn(`[${V}] akiraAnotarAccion falló:`, errAnota.message);
        }
      }
    } catch (err) {
      console.error(`[${V}] ejecutarAccion EXCEPTION:`, err);
      trySetAttr('accionResultado', JSON.stringify({
        ok: false,
        error: { message: 'No he podido completar la acción.' }
      }));
    }
  });
