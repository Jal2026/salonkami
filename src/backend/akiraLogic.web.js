 ...
 * ───────────────────────────────────────────────────────────────────────────
 *   v1.13.1 dejó de guardar el relato del modelo cuando una acción quedaba
 *   LISTA. Pero cuando la acción se llamaba y NO llegaba a lista —faltan
 *   datos, colisión, ambigüedad— sí se guardaba. Y ahí estaba el daño real:
 *   el modelo escribía "reserva confirmada", eso quedaba en el historial, y
 *   al turno siguiente lo leía como hecho propio y lo repetía con más
 *   seguridad. Caso real: cuatro turnos afirmando que la cita existía con la
 *   agenda vacía.
 *
 *   Ahora ese turno se guarda como lo que fue: acción no ejecutada, con su
 *   estado, y nada tocado en el salón.
 *
 * ───────────────────────────────────────────────────────────────────────────
 * CAMBIOS v1.13.1 → v1.14.0 — LA SESIÓN RECUERDA SU PLANO (patrón CENTRI)
 * ───────────────────────────────────────────────────────────────────────────
 *   Al reabrir una conversación —o al volver a la app en el móvil, donde se
 *   cierra sola con facilidad— el chip se quedaba donde estuviera. Viene de
 *   origen: la sesión nunca guardó su plano, ni aquí ni en CATHOVIA.
 *
 *   No es cosmético. El historial viaja ENTERO al modelo en cada pregunta:
 *   una conversación de Recepción reabierta con el chip en Asesor mete turnos
 *   de un plano dentro del prompt de otro.
 *
 *   Circuito completo, igual que en CENTRI (centriConsole v1.0.6):
 *     · aquí — `_crearSesion` guarda `modo`; `akiraAbrirChat` lo devuelve.
 *     · page code — viaja dentro del atributo `history`.
 *     · widget — `_applyHistory` mueve el chip.
 *
 *   ⛔ REQUIERE el campo `modo` (Texto) en AkiraSessions. Escribir a un campo
 *      inexistente NO falla: wixData ignora la clave en silencio.
 *
 * ───────────────────────────────────────────────────────────────────────────
 * CAMBIOS v1.13.0 → v1.13.1 — EL RELATO FALSO TAMPOCO SE GUARDA
 * ───────────────────────────────────────────────────────────────────────────
 *   El widget ya no pinta el texto del modelo cuando hay acción preparada,
 *   pero se seguía GUARDANDO en AkiraMessages. Reaparecía al reabrir el chat
 *   y, peor, volvía al modelo como historial: en el turno siguiente daba la
 *   reserva por hecha porque él mismo había escrito que lo estaba.
 *   Ahora se guarda una línea del sistema que dice lo que de verdad pasó:
 *   acción preparada, pendiente de confirmar, nada ejecutado.
 *
 * ───────────────────────────────────────────────────────────────────────────
 * CAMBIOS v1.12.0 → v1.13.0 — LO QUE SE EJECUTA SE ESCRIBE EN LA CONVERSACIÓN
 * ───────────────────────────────────────────────────────────────────────────
 *
 *   La acción la ejecuta el page code, fuera del hilo de AKIRA. Nadie se lo
 *   contaba a la conversación: la cita quedaba guardada y el siguiente turno
 *   seguía pidiendo la confirmación que ya se había dado.
 *
 *   `akiraAnotarAccion` escribe ese par de mensajes en la sesión: lo que hizo
 *   la persona y lo que ocurrió de verdad. Así el turno siguiente parte de un
 *   hecho, no de una expectativa.
 *
 *   Lo escribe el SISTEMA, no el modelo. Es la diferencia entre "la cita está
 *   creada" y "he creado la cita": lo primero solo se puede afirmar si la
 *   escritura devolvió ok.
 *
 * ───────────────────────────────────────────────────────────────────────────
 * CAMBIOS v1.11.0 → v1.12.0 — LA CONDUCTA VUELVE A AkiraAlignment
 * ───────────────────────────────────────────────────────────────────────────
 *
 *   v1.11.0 montaba en el system prompt un bloque de reglas leído de una
 *   columna de AkiraCapabilities. Era un segundo sitio donde se escribe cómo
 *   se comporta AKIRA, y encima sin interfaz: AkiraAlignment ya existe, ya es
 *   por plano y ya se edita desde el Entrenador.
 *
 *   Ese bloque desaparece. AkiraCapabilities describe la CAPACIDAD (qué es,
 *   qué parámetros, qué llama, si pide confirmación). AkiraAlignment describe
 *   la CONDUCTA, para el plano ASISTENTE igual que para ASESOR y AYUDA.
 *
 *   Requiere backend/akiraEjecutorLogic.web.js v3.0.0.
 *
 * ───────────────────────────────────────────────────────────────────────────
 * CAMBIOS v1.10.0 → v1.11.0 — LAS ACCIONES Y SUS REGLAS SALEN DEL CÓDIGO
 * ───────────────────────────────────────────────────────────────────────────
 *
 *   Consigna de Jal: "poner todo lo posible en CMS para que una funcionalidad
 *   nueva no sea siempre tocar código de AKIRA sino actualizar CMS".
 *
 *   v1.10.0 generaba las herramientas de acción desde un registro que vivía
 *   en akiraEjecutorLogic, y metía las reglas de comportamiento del plano
 *   ASISTENTE escritas a mano en este archivo. Las dos cosas eran código.
 *
 *   AHORA las dos salen de AkiraCapabilities:
 *     · El ESQUEMA de cada herramienta (nombre, descripción, parámetros,
 *       obligatorios) sale de su fila.
 *     · Las REGLAS de cada acción salen de la columna `reglas` de esa misma
 *       fila y se montan en el system prompt bajo el nombre de la acción.
 *       Aquí ya no hay ni una instrucción de comportamiento escrita a mano
 *       para el plano ASISTENTE.
 *
 *   Consecuencia: una acción nueva se da de alta en la colección y AKIRA la
 *   ve, sabe llamarla y sabe cómo comportarse con ella, sin publicar nada.
 *
 *   `listarAccionesCore(plano)` pasa a ser ASYNC (lee CMS), así que
 *   `_toolsDelPlano` también lo es y `askAkiraCore` la espera. Los nombres de
 *   acción vivos se calculan UNA vez por petición y viajan al bucle de
 *   herramientas, en vez de releer la colección en cada vuelta.
 *
 *   Requiere backend/akiraEjecutorLogic.web.js v2.0.0.
 *
 * ───────────────────────────────────────────────────────────────────────────
 * CAMBIOS v1.9.0 → v1.10.0 — PLANO ASISTENTE: AKIRA EJECUTA
 * ───────────────────────────────────────────────────────────────────────────
 *
 *   El plano `asistente` llevaba desde v1.5.0 en la lista de planos válidos
 *   sin uso: alcanzable por el endpoint público, sin chip en el widget, sin
 *   alignment publicado, y heredando por omisión el bloque de reglas del
 *   ASESOR — incluida la regla 5, "eres SOLO LECTURA". Era un ASESOR a medio
 *   vestir. Ahora es la recepcionista.
 *
 *   QUÉ CAMBIA
 *
 *   1) HERRAMIENTAS POR PLANO, NO SÍ/NO. `_planoUsaHerramientas` decidía si
 *      había herramientas; ahora `_toolsDelPlano` decide CUÁLES. ASESOR sigue
 *      con las dos de datos, AYUDA sigue sin ninguna, y ASISTENTE suma las de
 *      acción. `_callClaudeConHerramientas` recibe el array ya resuelto en vez
 *      de un booleano.
 *
 *   2) UNA HERRAMIENTA POR ACCIÓN, GENERADA DEL REGISTRO. El esquema sale de
 *      `listarAccionesCore()` de akiraEjecutorLogic. Añadir una capacidad al
 *      salón es añadir una entrada a aquel registro: aquí no hay que tocar
 *      nada, ni el prompt ni el enrutador. Un `switch` con cuarenta casos
 *      sería AkiraCapabilities otra vez.
 *
 *   3) EL MODELO PREPARA, NO ESCRIBE. Las herramientas de acción llaman a
 *      `prepararAccionCore`, que es solo lectura. La escritura vive en
 *      `ejecutarAccion` (Permissions.SiteMember) y la dispara el page code
 *      cuando una persona confirma en pantalla. Tres motivos, por peso:
 *        · /_functions/akiraAsk es PÚBLICO y sin autenticar. Mientras AKIRA
 *          era solo lectura el riesgo era coste; con escritura sería un
 *          endpoint anónimo de escritura sobre la agenda de producción.
 *        · La escritura se sale del techo de 14s del gateway y del tope de
 *          4 vueltas del bucle de herramientas.
 *        · Vuelve la confirmación antes de tocar la agenda.
 *
 *   4) LA PROPUESTA SUBE AL WIDGET. Cuando una preparación sale `listo`, el
 *      bucle la retiene y `askAkiraCore` la devuelve en `propuesta`, con la
 *      acción y los parámetros con los que se preparó. El widget pinta la
 *      burbuja de confirmación con ESE dato, no reconstruyendo nada del texto
 *      de la respuesta. Al modelo, en cambio, se le devuelve la preparación
 *      SIN el payload interno: son identificadores que no aportan a la
 *      conversación y gastan contexto.
 *
 *   5) REGLAS PROPIAS DEL PLANO. La regla 5 ("eres solo lectura") ya solo se
 *      envía a los planos sin acciones. ASISTENTE recibe en su lugar las
 *      suyas: hereda las reglas del interfaz que imita, pregunta lo que falta
 *      en vez de rellenarlo, y nunca fuerza un solape por su cuenta.
 *
 *   ADITIVO PARA ASESOR Y AYUDA. Ninguno de los dos ve una herramienta nueva
 *   ni una regla nueva. Si `akiraEjecutorLogic` no estuviera desplegado, el
 *   plano ASISTENTE se queda sin acciones y los otros dos siguen igual.
 *
 *   Requiere backend/akiraEjecutorLogic.web.js v1.0.0 y, por debajo,
 *   widgetPublicoLogic v0.11.0.
 *
...
// v1.10.0 — Registro de acciones del plano ASISTENTE. `listarAccionesCore` y
// `prepararAccionCore` son funciones PURAS (no webMethod) por el mismo motivo
// que askAkiraCore: el bucle de herramientas corre sin sesión de miembro.
 ...
 */
async function _callClaudeConHerramientas(model, apiKey, systemBlocks, messages, timeoutMs, tools, nombresAccion) {
  const startMs = Date.now();
  const convo = messages.slice();
  let cacheStats = { hit: 0, create: 0, input: 0, output: 0 };
  let consultas = 0;
  // v1.10.0 — `tools` llega ya resuelto por plano. null = sin herramientas.
  const usaTools = Array.isArray(tools) && tools.length > 0;
  // v1.11.0 — Se calculan UNA vez por petición en askAkiraCore. Antes se
  // releía la colección en cada vuelta del bucle.
  const setAcciones = new Set(nombresAccion || []);
  // v1.10.0 — Última preparación de acción que salió LISTA. Es lo que sube al
  // widget para pintar la burbuja de confirmación. Si el usuario encadena dos
  // peticiones en el mismo turno, manda la última.
  let propuesta = null;
  ...
  for (let vuelta = 0; vuelta < 4; vuelta++) {
    ...
    if (usaTools) payload.tools = tools;
    ...
    if (data.stop_reason !== 'tool_use' || toolUses.length === 0) {
      ...
      return { respuesta, timeMs: Date.now() - startMs, cacheStats, consultas, propuesta, accionIncompleta };
    }
    ...
    for (const tu of toolUses) {
      ...
      try {
        ...
        if (tu.name === 'consultar_configuracion_salon') {
        ...
        } else if (setAcciones.has(tu.name)) {
          // v1.10.0 — ACCIÓN. `prepararAccionCore` es SOLO LECTURA: resuelve
          ...
          if (prep && prep.estado === 'listo') {
            ...
            // v1.15.0 — Acción llamada que NO llegó a quedar lista. Se retiene
          ...
          // resumen y lo que falte por decidir, que es con lo que tiene que
          // hablar. (Capa de datos ≠ capa de conversación.)
          out = { ...prep };
          delete out.payload;
        } else {
...
async function _callClaudeConFallback(apiKey, systemBlocks, messages, tools, nombresAccion) {
  try {
    const r = await _callClaudeConHerramientas(MODEL_PRIMARY, apiKey, systemBlocks, messages, PRIMARY_TIMEOUT_MS, tools, nombresAccion);
    return { ...r, modeloUsado: MODEL_PRIMARY };
  } catch (err1) {
    console.warn(`${TAG} Sonnet falló, cayendo a Haiku: ${err1.message}`);
    try {
      const r = await _callClaudeConHerramientas(MODEL_FALLBACK, apiKey, systemBlocks, messages, FALLBACK_TIMEOUT_MS, tools, nombresAccion);
...
// v1.10.0 — Herramientas de ACCIÓN: solo el plano ASISTENTE. Es la
// recepcionista: hace lo que se haría pulsando en los interfaces. ASESOR
...
// akiraEjecutorLogic. Añadir una capacidad allí la hace visible aquí sin
// tocar este archivo ni el prompt.
//
// Si el ejecutor no estuviera desplegado, esto devuelve [] y el plano
// ASISTENTE se queda sin acciones en vez de romper la conversación.
// v1.11.0 — Las capacidades del plano salen de AkiraCapabilities. Si la
// colección no está o falla, se devuelve lista vacía: el plano se queda sin
// acciones, pero la conversación no se rompe y ASESOR/AYUDA no se enteran.
async function _capacidadesDelPlano(plano) {
  try {
    return (await listarAccionesCore(plano)) || [];
  } catch (e) {
    console.warn(`${TAG} ⚠️ capacidades no disponibles: ${e.message}`);
    return [];
  }
}

// Esquema de herramienta a partir de una fila. Todo sale de la fila: nada de
// lo que el modelo lee sobre la acción está escrito en este archivo.
function _toolDeCapacidad(a) {
  const ejemplos = (Array.isArray(a.ejemplos) && a.ejemplos.length)
    ? ` Ejemplos de lo que la dispara: ${a.ejemplos.slice(0, 4).join(' / ')}`
    : '';
  const nota = a.confirmacion
    ? ' Llamarla NO la ejecuta: devuelve una propuesta que una persona confirma en pantalla.'
    : '';
  return {
    name: a.accion,
    description: `${a.modulo ? '[' + a.modulo + '] ' : ''}${a.titulo}. ${a.descripcion}${nota}${ejemplos}`,
    input_schema: {
      type: 'object',
      properties: a.parametros || {},
      required: a.requeridos || []
    }
  };
}

// Juego de herramientas del plano. Devuelve null cuando no hay ninguna:
// `tools: []` es error de la API, hay que OMITIR la clave del body.
async function _toolsDelPlano(plano, capacidades) {
  if (!_planoUsaHerramientas(plano)) return null;
  const tools = [TOOL_CONSULTAR, _buildToolConfig()];
  if (_planoUsaAcciones(plano)) {
    for (const a of (capacidades || [])) tools.push(_toolDeCapacidad(a));
  }
  return tools;
}
...
function _identidadPorDefecto(plano, brand) {
  ...
  if (plano === 'asistente') {
    return `Eres AKIRA, la inteligencia artificial de ${brand}, integrada en KAMISUITE. ` +
      `Trabajas en modo ASISTENTE: eres una recepcionista más del salón. Además de consultar ` +
      `los datos, HACES cosas — reservar citas y todo lo que el equipo haría pulsando en las ` +
      `pantallas de KAMISUITE. Quien te habla está trabajando, muchas veces con una clienta ` +
      `delante: ve al grano, pregunta solo lo que falte y confirma lo que vas a hacer antes ` +
      `de hacerlo. Hablas en español, con criterio profesional y sin rodeos.`;
  }
...
async function _crearSesion(userId, userName, primeraQuery, modo) {
  const now = new Date();
  ...
  const registro = {
    ...
    messageCount: 0,
    // v1.14.0 — El plano con el que nació la conversación. Sin esto, al
    // reabrirla el chip se queda donde estuviera y se mezclan planos.
    modo: String(modo || PLANO_DEFECTO)
  };
...
export async function crearSesionCore({ userId, userName, query, modo } = {}) {
  try {
    // v1.14.0 — el plano viaja desde el widget para que la sesión nazca con él.
    const sessionId = await _crearSesion(userId, userName, query, _planoPedido(modo));
...
/**
 * v1.13.0 — Deja constancia en la conversación de una acción ya ejecutada.
 *
 * La escritura ocurre fuera de askAkiraCore (la dispara el page code con
 * sesión de miembro), así que sin esto el hilo se queda congelado en el
 * momento anterior a confirmar.
 *
 * Se guardan dos mensajes, como cualquier turno: el del usuario y el del
 * asistente. El texto del asistente lo redacta el SISTEMA a partir de lo que
 * devolvió la ejecución, nunca el modelo.
 */
export const akiraAnotarAccion = webMethod(
  Permissions.SiteMember,
  async ({ sessionId, textoUsuario, textoResultado } = {}) => {
    try {
      ...
      console.error(`${TAG} ❌ akiraAnotarAccion:`, e.message);
...
export async function askAkiraCore({ sessionId, query, userId, userName, modo }) {
  ...
  try {
    ...
    // v1.11.0 — capacidades del plano desde AkiraCapabilities. De ahí salen
    // el esquema de las herramientas de acción.
    const capacidades = _planoUsaAcciones(plano) ? await _capacidadesDelPlano(plano) : [];
    ...
    const tools = await _toolsDelPlano(plano, capacidades);
    const nombresAccion = capacidades.map(a => a.accion);
    ...
    let r;
    try {
      r = await _callClaudeConFallback(apiKey, systemBlocks, messages, tools, nombresAccion);
    } catch (err) {
    ...
    const { respuesta, modeloUsado, timeMs: apiMs, cacheStats, consultas, propuesta, accionIncompleta } = r;
    ...
    console.log(`${TAG} askAkiraCore OUT total=${totalMs}ms (prep=${prepMs}ms api=${apiMs}ms) modelo=${modeloUsado} consultas=${consultas} cache=${cacheStats.hit}/${cacheStats.create} len=${respuesta.length}${propuesta ? ` propuesta=${propuesta.accion}` : ''}`);

    // v1.10.0 — `propuesta` solo viaja cuando una acción quedó LISTA y espera
    // confirmación. El widget pinta la burbuja con ESTE dato, nunca
    // reinterpretando el texto de la respuesta. Sin propuesta, el contrato de
    // salida es idéntico al de v1.9.0.
    // v1.15.1 — Lo que se GUARDA y lo que se MUESTRA son cosas distintas.
    // `textoHistorial` es una nota interna para que el modelo no lea en el
    // turno siguiente que ya reservó; se llegó a pintar en pantalla y el
    // usuario vio un corchete técnico en vez de una respuesta. En pantalla va
    // SIEMPRE `respuesta`.
    const salida = { ok: true, respuesta, sessionId: effectiveSessionId };
    if (propuesta) salida.propuesta = propuesta;
    return salida;
...
export const akiraAbrirChat = webMethod(
  ...
  async ({ sessionId }) => {
    try {
      ...
      // v1.14.0 — El plano de la sesión viaja con el historial para que el
      // chip vuelva a su sitio. `null` = sesión anterior al campo: el widget
      // lo interpreta como "no toques el chip".
      let modo = null;
      try {
