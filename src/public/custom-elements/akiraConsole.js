 ...
 *   akiraLogic guarda en el historial una nota entre corchetes cuando una
 *   acción no se ejecutó, para que el modelo no la dé por hecha en el turno
 *   siguiente. Esa nota llegó a la pantalla y el usuario leyó un corchete
 *   técnico en vez de una respuesta. Se filtra en los DOS sitios donde se
 *   pinta texto: la respuesta en vivo y el historial al reabrir un chat.
 *
 * CAMBIOS v1.4.2 → v1.5.0 — EL CHIP VUELVE A SU SITIO Y LA VOZ ES UN MODO
 *
 *   1) PLANO PERSISTENTE (patrón CENTRI, centriConsole v1.0.6).
 *      Al reabrir una conversación —o al volver a la app en el móvil, que se
 *      cierra sola con facilidad— el chip se quedaba donde estuviera. La
 *      sesión ahora nace con su plano y `_applyHistory` lo restituye.
 *      NO se llama a _setPlano(): ese método abre chat nuevo a propósito y
 *      aquí borraría justo la conversación que se está restaurando.
 *      `modo` null = sesión anterior al campo: no se toca el chip.
 *
 *   2) LA VOZ ES UN MODO, NO UN TURNO.
 *      Había que volver a pulsar el micrófono para cada frase. Quien activa
 *      la voz quiere DIALOGAR hablando, no dictar una vez. Con la voz
 *      automática encendida (botón 🔊 de la barra), el micrófono se rearma
 *      solo en cuanto AKIRA termina de hablar, y sigue así hasta que se
 *      apaga ese botón.
 *      No se rearma mientras suena el audio: se oiría a sí misma.
 *
 * CAMBIOS v1.4.1 → v1.4.2 — EL MODELO NO PUEDE DAR NADA POR HECHO.
 *   Cuando la respuesta trae una acción preparada, el texto del modelo no se
 *   pinta: la respuesta es la tarjeta. Llegó a escribir "¡Hecho! Cita
 *   reservada" con la tarjeta aún sin confirmar y sin que nadie hubiera
 *   escrito en la agenda. Ahora lo único que puede afirmar que algo está
 *   hecho es el resultado real de la ejecución.
 *
 * CAMBIOS v1.4.0 → v1.4.1 — La confirmación viaja con su sesión, para que el
 *   page code pueda anotar en el hilo lo que se ha ejecutado de verdad.
 *
 * CAMBIOS v1.3.0 → v1.4.0 — SE CONFIRMA HABLANDO, NO SOLO PULSANDO.
 *
 *   El chip pasa a llamarse RECEPCIÓN (el plano interno sigue siendo
 *   'asistente').
 *
 *   Y lo importante: con una tarjeta abierta, escribir o decir "confirmar"
 *   vale exactamente igual que pulsar el botón. El botón deja de ser la
 *   puerta y pasa a ser un atajo para quien está delante de la pantalla.
 *
 *   POR QUÉ: por teléfono no hay botón. Si la única forma de confirmar es
 *   pulsar, AKIRA no puede atender una reserva hablando — que es a donde va
 *   esto. La confirmación tiene que ser conversación.
 *
 *   CÓMO: si hay tarjeta pendiente y el mensaje es una confirmación, NO se
 *   llama a AKIRA: se ejecuta esa tarjeta, igual que el botón. Así no se
 *   pinta una segunda tarjeta ni el modelo puede contar que ya ha reservado
 *   cuando no ha reservado nadie. Quien dice que está hecho es la ejecución.
 *
 *   Si el mensaje es una negativa, se cierra la tarjeta y tampoco se llama.
 *   Cualquier otra cosa sigue su curso normal y la tarjeta se queda ahí.
 *
 * CAMBIOS v1.2.0 → v1.3.0 — PLANO ASISTENTE Y BURBUJA DE CONFIRMACIÓN.
 *
 *   Tercer chip: ASISTENTE. Es la recepcionista — además de consultar, hace
 *   cosas. Reservar es la primera.
 *
 *   AKIRA NO ESCRIBE SOLA. Cuando el backend deja una acción lista, la
 *   respuesta trae `propuesta` y aquí se pinta una tarjeta con el resumen y
 *   dos botones. Hasta que alguien pulsa Confirmar no se escribe nada.
 *
 *   LA TARJETA SE PINTA CON EL DATO, NO CON EL TEXTO. `propuesta` llega
 *   estructurada desde el backend: servicio, cliente, profesional, día, hora
 *   y precio son campos, no frases que haya que interpretar. El texto de
 *   AKIRA acompaña; la tarjeta manda.
 *
 *   Si la propuesta viene marcada como solape, la tarjeta lo avisa antes de
 *   los botones. Confirmar ahí es autorizar el solape.
 *
 *   Al confirmar se emite `akira-ejecutar-accion` al page code, que es quien
 *   escribe (con sesión de miembro). El resultado vuelve por el atributo
 *   `accionResultado`, declarado en las dos grafías por la lección de v1.2.0:
 *   el DOM minusculiza los nombres de atributo.
 *
 *   Estética: reutiliza los tokens del skin activo. No se añade ninguna
 *   variable de color nueva ni se toca el CSS existente.
 *
...
(function () {
  ...
  const BRAND_PLANOS = {
    ...
    asistente: {
      sub:          'Recepción',
      welcomeTitle: 'AKIRA · Recepción',
      welcome:      'Dime qué hay que hacer y lo hago. Reservar una cita, por ejemplo: cuéntame el servicio, para quién y cuándo. Te enseño lo que voy a hacer antes de hacerlo.',
      placeholder:  '¿Qué hacemos?',
      thinking:     'Mirando la agenda…'
    }
  };
  ...
  const PLANOS = [
    ...
    { id: 'asistente', label: 'Recepción', title: 'Hace cosas en el salón: reservar y más' }
  ];

  /* v1.4.0 — Confirmar y cancelar de viva voz.
     Se compara normalizado (sin tildes, sin signos, sin mayúsculas) y por
     igualdad exacta: un "confirmar" suelto confirma, pero "no confirmes eso"
     no, porque no coincide entero. Lo dudoso va a AKIRA como siempre. */
  const PALABRAS_SI = [
    'confirmar', 'confirma', 'confirmo', 'confirmado', 'si', 'si confirmo',
    'vale', 'ok', 'okey', 'de acuerdo', 'adelante', 'dale', 'hazlo',
    'correcto', 'perfecto', 'venga', 'eso es', 'si por favor', 'resérvala',
    'reservala', 'confirmalo', 'confirmala'
  ];
  const PALABRAS_NO = [
    'cancelar', 'cancela', 'cancelalo', 'no', 'no gracias', 'dejalo',
    'dejalo estar', 'anula', 'anulalo', 'olvidalo', 'mejor no', 'para'
  ];

  function normalizarOrden(txt) {
    return String(txt || '')
  ...
  class AkiraConsole extends HTMLElement {
    ...
    static get observedAttributes() {
      ...
      return [
        ...
        'ttsAudio', 'ttsError', 'systemError',
        // v1.3.0 — resultado de una acción ejecutada por el page code.
    ...
    constructor() {
      ...
      this._userId    = '';
      this._userName  = '';
      this._modo      = 'asesor';        // plano: 'asesor' | 'ayuda' | 'asistente'
      this._propuestaBlock = null;       // tarjeta de confirmación pendiente
      this._propuestaDatos = null;       // { accion, params } de esa tarjeta
    ...
    attributeChangedCallback(name, oldVal, newVal) {
      ...
      if (attr === 'accionresultado') this._applyAccionResultado(payload);
    }
    ...
    _applyHistory(payload) {
      ...
      // v1.5.0 — El chip vuelve al plano con el que nació la conversación.
      //
      // ⚠️ NO se llama a _setPlano(): ese método abre chat nuevo A PROPÓSITO,
      //    y aquí borraría justo la conversación que se está restaurando.
      ...
      if (mensajes.length === 0) {
        ...
        mensajes.forEach(m => {
          if (m.rol === 'user') { this._appendUser(m.contenido, false); return; }
          // v1.5.1 — Las notas internas van entre corchetes y son para el
          // modelo, no para la pantalla. Al reabrir un chat se pintarían tal
          // cual, que es como el usuario acabó leyendo "[Acción NO ejecutada.
          // Estado: faltan_datos…]" en mitad de su conversación.
          const t = String(m.contenido || '');
          if (/^\s*\[/.test(t) && /\]\s*$/.test(t)) return;
          this._appendAssistant(t, false);
        });
    ...
    _handleResponse(payload) {
      ...
      if (payload.error && !payload.respuesta) {
        ...
        this._stopPolling();
        this._lastErrorBlock = null;
        // v1.4.2 — Si hay una acción preparada, el texto del modelo NO se
        // pinta. Mandaba la tarjeta y el relato sobraba, pero además el
        // modelo llegó a escribir "¡Hecho! Cita reservada" cuando no había
        // escrito nadie: quien había preparado la acción era el backend, y la
        // escritura ni siquiera se había pedido. Mientras ese texto pueda
        // verse, AKIRA puede dar por hecho lo que no ha ocurrido.
        //
        // Con la propuesta en mano, la respuesta ES la tarjeta. Lo único que
        // puede afirmar que algo está hecho es el resultado de la ejecución,
        // que se pinta en _applyAccionResultado.
        if (payload.propuesta) {
          this._appendPropuesta(payload.propuesta);
        } else if (/^\s*\[/.test(text) && /\]\s*$/.test(text)) {
          // v1.5.1 — Red de seguridad: las notas internas van entre corchetes
          // y son para el modelo, no para la pantalla. Una se coló y el
          // usuario leyó "[Acción reservar_cita NO ejecutada. Estado:
          // faltan_datos...]" en mitad de la conversación.
          console.warn(`${TAG} nota interna interceptada, no se pinta`);
        } else {
    ...
    _sendQuery(query) {
      ...
      // v1.4.0 — Con una tarjeta abierta, confirmar o cancelar de palabra vale
      // igual que pulsar. No se llama a AKIRA: se ejecuta la tarjeta. Así no
      // aparece una segunda tarjeta ni el modelo puede narrar una reserva que
      // no ha hecho nadie.
      if (this._propuestaBlock && this._propuestaDatos) {
        const orden = normalizarOrden(query);
        if (PALABRAS_SI.indexOf(orden) >= 0) {
          this._clearWelcome();
          this._appendUser(query, true);
          this._propuestaDatos.textoUsuario = query;
          this._confirmarPropuesta();
          return;
        }
        if (PALABRAS_NO.indexOf(orden) >= 0) {
          this._clearWelcome();
          this._appendUser(query, true);
          this._cerrarPropuesta(this._propuestaBlock, 'Cancelado. No he tocado nada.');
      ...
      try {
        const res = await fetch('/_functions/akiraNuevaSesion', {
          ...
          body: JSON.stringify({
            ...
            query: query,
            modo: this._modo          // v1.5.0 — la sesión nace con su plano
          })
        });
        if (!res.ok) {
    ...
    async _fetchAkiraAsk(query, messageId) {
      try {
        ...
        this._handleResponse({
          ...
          propuesta: data.propuesta || null,   // v1.3.0
    ...
    /* ─────────────────────────────────────────────────────────────────────
       v1.3.0 · TARJETA DE CONFIRMACIÓN DE ACCIÓN

       AKIRA propone; una persona decide. Esta tarjeta es ese punto: hasta
       que alguien pulsa Confirmar no se escribe nada en el salón.

       Se pinta con `propuesta`, que llega estructurada del backend. El texto
       de AKIRA acompaña, pero lo que se confirma es este dato.
       ───────────────────────────────────────────────────────────────────── */
    _appendPropuesta(prop) {
      if (!prop || !prop.accion || !prop.resumen) return;
      const r = prop.resumen;

      const filas = [];
      const fila = (etq, val) => {
        if (val === null || val === undefined || val === '') return;
        filas.push(`<div class="prop-row"><span class="prop-k">${this._escape(etq)}</span><span class="prop-v">${this._escape(val)}</span></div>`);
      };

      fila('Servicio',  r.variante ? `${r.servicio} · ${r.variante}` : r.servicio);
      if (Array.isArray(r.complementos) && r.complementos.length) {
        fila('Con', r.complementos.join(' · '));
      }
      fila('Cliente',      r.cliente);
      fila('Profesional',  r.profesional);
      fila('Día y hora',   [r.fecha, r.hora].filter(Boolean).join('  ·  '));
      if (r.duracionMin) fila('Duración', `${r.duracionMin} min`);
      if (r.aValorar) fila('Precio', 'A valorar');
      else if (typeof r.precio === 'number') fila('Precio', `${r.precio.toFixed(2)} €`);

      const avisoSolape = (prop.aviso === 'solape')
        ? `<div class="prop-warn">Esta cita se solapa con otra que ya hay en la agenda. Al confirmar, la creas igualmente.</div>`
        : '';

      const el = document.createElement('div');
      el.className = 'turn turn-prop';
      el.innerHTML = `
        <div class="prop-box${prop.aviso === 'solape' ? ' prop-box-warn' : ''}">
          <div class="prop-head">Voy a hacer esto</div>
          <div class="prop-grid">${filas.join('')}</div>
          ${avisoSolape}
          <div class="prop-actions">
            <button class="prop-btn prop-btn-ok" type="button">Confirmar</button>
            <button class="prop-btn prop-btn-no" type="button">Cancelar</button>
          </div>
          <div class="prop-status" hidden></div>
        </div>
      `;
      this.shadowRoot.getElementById('messages').appendChild(el);

      const btnOk = el.querySelector('.prop-btn-ok');
      const btnNo = el.querySelector('.prop-btn-no');

      btnNo.addEventListener('click', () => {
        this._cerrarPropuesta(el, 'Cancelado. No he tocado nada.');
        this._propuestaDatos = null;
      });

      // El botón hace lo MISMO que decir "confirmar": los dos entran por
      // _confirmarPropuesta. Un solo camino, un solo sitio donde equivocarse.
      btnOk.addEventListener('click', () => this._confirmarPropuesta());

      // Solo puede haber una tarjeta viva. Si llegara otra, la anterior se
      // cierra: si no, "confirmar" sería ambiguo.
      if (this._propuestaBlock && this._propuestaBlock !== el) {
        this._cerrarPropuesta(this._propuestaBlock, 'Sustituida por la propuesta siguiente.');
      }
      this._propuestaBlock = el;
      this._propuestaDatos = { accion: prop.accion, params: prop.params || {} };

      this._scrollBottom();
      return el;
    }

    /* Único camino de confirmación: lo usan el botón y la palabra hablada. */
    _confirmarPropuesta() {
      const el = this._propuestaBlock;
      const datos = this._propuestaDatos;
      if (!el || !datos) return;

      // Se bloquea antes de emitir: un doble clic o un "confirmar" repetido
      // crearían la cita dos veces.
      const btnOk = el.querySelector('.prop-btn-ok');
      const btnNo = el.querySelector('.prop-btn-no');
      if (btnOk) btnOk.disabled = true;
      if (btnNo) btnNo.disabled = true;
      this._setPropStatus(el, 'Guardando…');
      this._scrollBottom();

      // v1.4.1 — Viaja la sesión para poder anotar en el hilo lo ejecutado.
      this._emit('akira-ejecutar-accion', {
        accion: datos.accion,
        params: datos.params,
        sessionId: this._sessionId,
        textoUsuario: datos.textoUsuario || 'Confirmar'
      });
    }

    _setPropStatus(el, texto) {
      const st = el && el.querySelector('.prop-status');
      if (!st) return;
      st.textContent = texto;
      st.hidden = false;
    }

    _cerrarPropuesta(el, texto) {
      if (!el) return;
      const acciones = el.querySelector('.prop-actions');
      if (acciones) acciones.remove();
      this._setPropStatus(el, texto);
      const box = el.querySelector('.prop-box');
      if (box) box.classList.add('prop-box-cerrada');
      if (this._propuestaBlock === el) this._propuestaBlock = null;
      this._scrollBottom();
    }

    /* Resultado de la escritura, que hace el page code con sesión de miembro. */
    _applyAccionResultado(res) {
      const el = this._propuestaBlock;
      if (!el) {
        console.warn(`${TAG} accionResultado sin tarjeta pendiente`);
        return;
      }
      if (res && res.ok) {
      ...
      // No se ha escrito: se devuelven los botones para poder reintentar.
      const msg = (res && res.error && res.error.message)
               || (typeof res?.error === 'string' ? res.error : null)
               || 'No he podido hacerlo.';
      const box = el.querySelector('.prop-box');
      if (box) box.classList.add('prop-box-warn');
      this._setPropStatus(el, msg);
      const btnOk = el.querySelector('.prop-btn-ok');
      const btnNo = el.querySelector('.prop-btn-no');
    ...
    _playAudioBase64(base64, mimeType, messageId) {
      ...
      audio.onended = () => {
        ...
        this._playingMessageId = null;
        this._rearmarMicSiProcede();   // v1.5.0
      };
      audio.onerror = () => {
    ...
    /* v1.5.0 — La voz es un modo: mientras el botón 🔊 esté encendido, el
       micrófono se rearma solo en cuanto AKIRA termina de hablar. Nunca antes:
       con el audio sonando se oiría a sí misma. */
    _rearmarMicSiProcede() {
      if (!this._ttsAutoPlay) return;      // el usuario apagó la voz
      if (!this._SR) return;               // navegador sin reconocimiento
      if (this._isListening || this._pending) return;
      setTimeout(() => {
        if (this._ttsAutoPlay && !this._isListening && !this._pending) {
          this._startMic();
        }
      }, 350);
    }
    ...
    _toggleTtsAutoPlay() {
      ...
      // v1.5.0 — Apagar la voz apaga también el micrófono: es un solo modo.
      if (!this._ttsAutoPlay && this._isListening) this._stopMic();
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  /* v1.3.0 · TARJETA DE CONFIRMACIÓN DE ACCIÓN.
     Solo tokens del skin activo: ninguna variable de color nueva. */
  .turn-prop { align-items: flex-start; }
  .prop-box {
    background: var(--card); border: 1px solid var(--hairline);
    border-radius: 14px; padding: 16px 18px; max-width: 78%;
    display: flex; flex-direction: column; gap: 12px;
  }
  .prop-box-warn { border-color: rgba(160,48,48,.35); }
  .prop-box-cerrada { opacity: .72; }
  .prop-head {
    font-size: 12px; font-weight: 600; letter-spacing: .07em;
    text-transform: uppercase; color: var(--muted);
  }
  .prop-grid { display: flex; flex-direction: column; gap: 7px; }
  .prop-row { display: flex; gap: 12px; align-items: baseline; font-size: 14.5px; }
  .prop-k { color: var(--muted); min-width: 92px; flex-shrink: 0; font-size: 13px; }
  .prop-v { color: var(--ink); font-weight: 500; }
  .prop-warn {
    background: rgba(160,48,48,.07); border: 1px solid rgba(160,48,48,.24);
    color: #8a2525; border-radius: 10px; padding: 10px 12px;
    font-size: 13.5px; line-height: 1.5;
  }
  .prop-actions { display: flex; gap: 9px; }
  .prop-btn {
    border-radius: 9px; padding: 9px 17px; font-size: 13.5px; font-weight: 600;
    cursor: pointer; font-family: var(--font-body); border: 1px solid transparent;
  }
  .prop-btn:disabled { opacity: .5; cursor: default; }
  .prop-btn-ok { background: var(--accent); color: var(--accent-ink); }
  .prop-btn-no { background: transparent; color: var(--muted); border-color: var(--hairline); }
  .prop-btn-no:hover:not(:disabled) { color: var(--ink); }
  .prop-status { font-size: 13px; color: var(--muted); font-style: italic; }
